...
const Landing = () => {
  ...
  const { config, error } = usePrepareContractWrite({
    address: "0x8AC95Fa1089f1b2F5bdC3Cb20810b68182f7128F",
    abi: layerabi.abi,
    functionName: "mint",
    args: [address, search],
  });
      ...
      </div>
      {
        <>
          <div className="flex flex-row justify-center items-center gap-2">
            <input
              value={parseFloat(formatEther(search))}
              onChange={handleSearch}
              autoComplete="off"
              className=" lg:text-4xl md:text-3xl sm:text-2xl text-xl xl:w-[250px] lg:w-[600px] md:w-[500px] sm:w-96 w-56 pl-6 py-3 border-black placeholder:text-black px-3 rounded-full border-4"
              type="text"
              ></input>
            <div className="flex bg-black rounded-full  h-16 self-center pr-[2px] pl-[4px] group hover:scale-105 hover:pb-1 cursor-pointer mx-1">
              <button
