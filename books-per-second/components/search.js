...
const Search = () => {
  ...
  return (
    <div className='flex flex-col justify-center items-center '>
            ...
            <input value={searchQuery} onChange={handleChange} autoComplete='off' className=' lg:text-4xl md:text-3xl sm:text-2xl text-xl xl:w-[1200px] lg:w-[600px] md:w-[500px] sm:w-96 w-56 py-3 border-black placeholder:text-black px-3 rounded-full border-4' placeholder='Search for books' type='text'></input>
        ...
        <h1  className='hover:scale-105 select-none mt-16 font-Lato text-2xl lg:text-7xl md:text-5xl sm:text-5xl'>Have a good read ⸂⸂⸜(രᴗര๑)⸝⸃⸃</h1>
        <div>
      ...
      <ul className="mt-20">
        {books.map((book, index) => (
          <li key={index}>
            ...
            <br />
            <img className="my-2" src={book.thumbnail} alt={`Thumbnail for ${book.title}`} />
          </li>
        ))}
      </ul>
    </div>
